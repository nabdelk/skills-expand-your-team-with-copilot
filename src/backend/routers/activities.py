...
def get_activities(
    ...
    - difficulty: Filter activities by difficulty level ('Beginner', 'Intermediate', 'Advanced', or 'no-level' for no difficulty)
    """
    ...
    if difficulty:
        if difficulty == "no-level":
            # "no-level" means activities without a difficulty field
